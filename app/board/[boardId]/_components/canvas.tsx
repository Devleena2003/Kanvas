"use client"
import { useCanRedo, useCanUndo, useHistory, useOthersMapped, useSelf, useStorage } from "@liveblocks/react";
...
export const Canvas = ({ boardId }: CanvasProps) => {
    ...
    const updateSelectionNet = useMutation(({ storage, setMyPresence },
    ...
    const continueDrawing = useMutation(({ self, setMyPresence },
        point: Point,
    ...
    const insertPath=useMutation(({ storage, self, setMyPresence }) => {
        ...
        const { pencilDraft } = self.presence
        if (pencilDraft == null ||
            pencilDraft.length < 2 || liveLayers.size >= MAX_LAYERS) {
    ...
    const resizeSelectedLayer = useMutation(({ storage, self }, point: Point) => {
        ...
        const bounds = resizeBounds(
            canvasState.initialBounds, canvasState.corner, point
        );
        const liveLayers = storage.get("layers");
    ...
    const onResizeHandlerPointerDown = useCallback((corner: Side,
        initialBounds: XYWH) => {
        history.pause();
        setCanvasState({
    ...
    return (
        <main className="h-full w-full relative bg-neutral-100 touch-none">
            ...
            <Toolbar canvasState={canvasState} setCanvasState={setCanvasState}
                redo={history.redo} undo={history.undo} canRedo={canRedo} canUndo={canUndo} />
